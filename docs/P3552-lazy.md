...
[11]{.pnum} Let `sched` be an object of type `any_scehduler` and let `sndr` be an object of type
`@_any-sender@` obtained from `schedule(sched)`. Then `get_completion_scheduler<set_value_t>(get_env(sndr)) == sched` is `true`.
