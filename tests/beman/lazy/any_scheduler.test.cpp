    ...
    struct thread_context {
        ...
        struct scheduler {
            ...
            struct state: base {
                ...
                template <typename R>
                state(auto c, R&& r)
                    : ctxt(c), receiver(std::forward<R>(r))
                {
                }
            ...
            struct env {
                thread_context* ctxt;
                scheduler query(ex::get_completion_scheduler_t<ex::set_value_t> const&) const noexcept {
...
int main() {
    ...
    assert(id1 != id2);
    ex::sync_wait(ex::schedule(ly::detail::any_scheduler(sched1)) | ex::then([&id1](){ assert(id1 == std::this_thread::get_id()); }));
    ex::sync_wait(ex::schedule(ly::detail::any_scheduler(sched2)) | ex::then([&id2](){ assert(id2 == std::this_thread::get_id()); }));
}
